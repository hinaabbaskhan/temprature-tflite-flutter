...
void main() {
  runApp(const ProviderScope(child: TempratureApp()));
}

class TempratureApp extends StatelessWidget {
  const TempratureApp({super.key});
      ...
      children: [
        ...
        const Center(
          child: Column(
            ...
            children: [TempInfoDisplay(), TempMLDisplay()],
          ),
        ),
...
class TempSlider extends ConsumerWidget {
  ...
  const TempSlider({required this.initialYPosition, super.key});
    ...
    return Stack(children: [
      ...
      Container(
          color: Color.fromRGBO(
              (tempValue.toInt() + 75), 0, 255 - (tempValue.toInt() + 75), 1)),
        ...
        child: Container(
          decoration: BoxDecoration(
              gradient: LinearGradient(
                colors: [
                  Color.fromRGBO((tempValue.toInt() + 75), 0,
                      255 - (tempValue.toInt() + 75), 1),
                  Color.fromRGBO((tempValue.toInt() + 75), 0,
                      255 - (tempValue.toInt() + 75), 0.5),
                ],
...
class TempSlideDetector extends ConsumerStatefulWidget {
  final double initialYPosition;
  const TempSlideDetector({required this.initialYPosition, super.key});
